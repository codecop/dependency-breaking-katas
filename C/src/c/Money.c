...
const C_Money* C_Money_createFromLongDouble(long double value)
{
    C_Money* money = malloc(sizeof(C_Money));
    if (money != NULL) {
        money->value = value;
...
{
    return C_Money_createFromLongDouble(self->value + other->value);
}

const C_Money* C_Money_percentage(const C_Money* self, int p)
{
    return C_Money_createFromLongDouble(self->value * p / 100);
}
